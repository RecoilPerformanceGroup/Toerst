...
#include <ofxCocoaPlugins/Keystoner.h>

@implementation DustVideoPlayer
...
-(void) setup{	
	...
	dispatch_async(dispatch_get_main_queue(), ^{	
		...
		for(int i=0;i<NUMVIDEOS;i++){
			[movie[i] retain];
			[movie[i] stop];
			[movie[i] setAttribute:[NSNumber numberWithBool:YES] forKey:QTMovieLoopsAttribute];
...
-(void) draw:(NSDictionary*)drawingInformation{
	if(PropI(@"video") > 0 && PropI(@"video") <= NUMVIDEOS){
		...
		if(currentFrame[i] != nil ){		
			...
			ofSetColor(255.0*PropF(@"colorR")*PropF(@"alpha"),255.0*PropF(@"colorG")*PropF(@"alpha"), 255.0*PropF(@"colorB")*PropF(@"alpha"), 255);
			glPushMatrix();
			
			int projector = 0;
			[GetPlugin(Keystoner)  applySurface:@"Floor" projectorNumber:projector viewNumber:ViewNumber];

			
			float aspect;
			//if(i == 0){
				aspect = Aspect(@"Floor",projector);	
				glBegin(GL_QUADS);{
